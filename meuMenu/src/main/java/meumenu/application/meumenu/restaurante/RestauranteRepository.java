...
public interface RestauranteRepository extends JpaRepository<Restaurante, Integer>{
    ...
    @Query(value = "SELECT r.nome, r.id, (SELECT nome_foto FROM restaurante_foto WHERE fk_restaurante = :fkRestaurante LIMIT 1) AS nomeFoto, " +
            "(SELECT SUM(nt_comida + nt_ambiente + nt_atendimento) / (SELECT COUNT(fk_restaurante) * 3 FROM review WHERE fk_restaurante = :fkRestaurante) " +
            "FROM review WHERE fk_restaurante = :fkRestaurante) AS media " +
